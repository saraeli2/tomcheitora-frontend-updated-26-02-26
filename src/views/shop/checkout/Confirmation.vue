<script setup>
const props = defineProps({
  currentStep: {
    type: Number,
    required: false,
  },
  checkoutData: {
    type: null,
    required: true,
  },
  shopInfo: {
    type: null,
    required: true,
  },
})

const emit = defineEmits([
  'update:currentStep',
  'update:checkout-data',
])

const checkoutAddressDataLocal = ref(props.checkoutData)
const deliveryCharge = ref(0)

const selectedDeliveryAddress = computed(() => {
  return props.checkoutData.addresses.filter(address => {
    return address.value === props.checkoutData.deliveryAddress
  })
})

const getDeliveyCharge = () =>{
 

  if(checkoutAddressDataLocal.value.deliveryType == 'inside_dhaka'){
    deliveryCharge.value = props.shopInfo.inside_dhaka
    
    return props.shopInfo.inside_dhaka
  }else{
    return props.shopInfo.outside_dhaka
  }
  
}

const totalCost = computed(() => {
  return props.checkoutData.cartItems.reduce((acc, item) => {
    return acc + (item.online_price || item.retail_price) * item.quantity
  }, 0)
})

const resolveDeliveryMethod = computed(() => {
  if (props.checkoutData.deliverySpeed === 'overnight')
    return {
      method: 'Overnight Delivery',
      desc: 'In 1 business day.',
    }
  else if (props.checkoutData.deliverySpeed === 'express')
    return {
      method: 'Express Delivery',
      desc: 'Normally in 3-4 business days',
    }
  else
    return {
      method: 'Standard Delivery',
      desc: 'Normally in 1 Week',
    }
})

const customerInfo = JSON.parse(localStorage.getItem('CUSTOMER_INFO')) || {}
</script>

<template>
  <section class="text-base confirm_page">
    <div class="text-center">
      <h5 class="text-h5 mb-3">
        Thank You! 😇
      </h5>
      <p>
        Your order <span class="text-primary">#1536548131</span> has been placed!
      </p>
      <p class="mb-0">
        We sent an email to <span class="text-primary">{{ customerInfo ? customerInfo.email : '-' }}</span> with your order confirmation and receipt.
      </p>
      <p>If the email hasn't arrived within two minutes, please check your spam folder to see if the email was routed there.</p>
      <!--
        <div class="d-flex align-center gap-2 justify-center">
        <VIcon
        size="20"
        icon="tabler-clock"
        />
        <span>Time placed: 25/05/2020 13:35pm</span>
        </div> 
      -->
    </div>

    <VRow class="border rounded ma-0 mt-6">
      <VCol
        cols="12"
        md="6"
        class="pa-5"
        :class="$vuetify.display.mdAndUp ? 'border-e' : 'border-b'"
      >
        <div class="d-flex align-center gap-2 text-high-emphasis mb-4">
          <VIcon icon="tabler-credit-card" />
          <span class="text-base font-weight-medium">
            Billing Address
          </span>
        </div>


        <p class="mb-1">
          {{ customerInfo ? customerInfo.name : '-' }}
        </p>
        <p class="mb-4">
          {{ customerInfo ? customerInfo.address : '-' }}
        </p>
        <span class="font-weight-medium">{{ customerInfo ? customerInfo.email : '-' }}</span>
      </VCol>

      <VCol
        cols="12"
        md="6"
        class="pa-5"
        style="padding-inline-start: 10px!important;"
      >
        <div class="d-flex align-center gap-2 text-high-emphasis mb-4">
          <VIcon icon="tabler-ship" />
          <span class="text-base font-weight-medium">
            Shipping Method
          </span>
        </div>

        <p class="font-weight-medium">
          Cash On Delivery
        </p>
      </VCol>
    </VRow>

    <VRow>
      <VCol
        cols="12"
        md="9"
      >
        <!-- 👉 cart items -->
        <div class="border rounded">
          <template
            v-for="(item, index) in props.checkoutData.cartItems"
            :key="item.name"
          >
            <div
              class="d-flex align-start gap-3 pa-5 position-relative flex-column flex-sm-row"
              :class="index ? 'border-t' : ''"
            >
              <div>
                <VImg
                  width="80"
                  :src="item.image"
                />
              </div>

              <div
                class="d-flex w-100 pt-3"
                :class="$vuetify.display.width <= 700 ? 'flex-column' : 'flex-row'"
              >
                <div>
                  <h6 class="text-base font-weight-regular mb-4">
                    {{ item.name }}
                  </h6>
                  <div class="d-flex align-center text-no-wrap gap-2 text-base">
                    <VChip
                      color="success"
                      label
                    >
                      <span>
                        In Stock
                      </span>
                    </VChip>
                  </div>
                </div>

                <VSpacer />

                <div
                  class="d-flex flex-column justify-space-between mt-3"
                  :class="$vuetify.display.width <= 700 ? 'text-start' : 'text-end'"
                >
                  <p class="mb-0">
                    <span class="text-primary">${{ item.online_price || item.retail_price }}</span>
                  </p>
                  <p class="mb-0">
                    <span>{{ item.quantity }} x ${{ item.online_price || item.retail_price }}</span>
                  </p>
                </div>
              </div>
            </div>
          </template>
        </div>
      </VCol>

      <VCol
        cols="12"
        md="3"
      >
        <div class="border rounded">
          <div class="border-b pa-5">
            <h6 class="text-base font-weight-medium mb-3">
              Price Details
            </h6>

            <div class="d-flex align-center justify-space-between text-sm mb-3">
              <span class="text-high-emphasis">Order Total</span>
              <span>${{ totalCost }}.00</span>
            </div>

            <div class="d-flex justify-space-between mb-2">
              <span>Delivery charge</span>
              <span>${{ getDeliveyCharge() }}</span>
            </div>
          </div>
          <div class="d-flex align-center justify-space-between text-high-emphasis font-weight-medium px-5 py-3">
            <span>Total</span>
            <span>${{ parseFloat(totalCost) + parseFloat(getDeliveyCharge()) }}.00</span>
          </div>
        </div>
      </VCol>
    </VRow>
  </section>
</template>
