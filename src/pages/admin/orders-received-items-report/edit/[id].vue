...
const submit = async () => {
  try {
    ...
    await nextTick(() => {
      ...
      router.push({ name: 'admin-orders-received-items-report-detail-id', params: { id: route.params.id } })
    })
  } catch (err) {
    console.log(err)
  }
    ...
    <VRow>
      ...
      >
        <VBreadcrumbs
          ...
          :items="[{ title: 'עדכון חוסרים\עודפים', to: { name: 'admin-orders-received-items-report' }, class: 'text-primary' }, { title: $t('Order Details') + ' ' + orderData._id, to: { name: 'admin-orders-weight-update-detail-id', params: { id: orderData._id } }, class: 'text-primary' }, { title: $t('Edit Order') }]"
        />
      </VCol>
    </VRow>
